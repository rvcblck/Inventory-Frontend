...
export class SupplierRequestComponent implements OnInit, AfterViewInit {
  requestData!: any;
  requestorData!: any;
  inventoryData!: any;
  usersData!: any;
  ...
  filterRequestorData(requestData: any[], usersData: any[], status: string) {
    ...
    const filteredData = requestData
      ...
      .filter((user: any) => user.from === admin_id); //only admin
  ...
  approveOrder() {
    ...
    const supplier_id = localStorage.getItem('user_id');
    const orderData = {
      data: orderItems,
